...
def do_op_instrumentation_benchmarks(
    data_frame_rows: int,
    operator_type: OperatorBenchmarkType,
    repeats: int = 1,
) -> dict[str, list[float]]:
    """
    Do the projection benchmarks
    """
    code_to_benchmark = get_code_for_op_benchmark(
        data_frame_rows, operator_type
    )
    benchmark_results = exec_benchmarks_empty_inspection(
        code_to_benchmark, repeats
    )
    return benchmark_results


def do_op_inspections_benchmarks(
    data_frame_rows: int,
    operator_type: OperatorBenchmarkType,
    repeats: int = 1,
) -> dict[str, list[float]]:
    ...
    benchmark_results = exec_benchmarks_nonempty_inspection(
        code_to_benchmark, repeats
    )
    return benchmark_results


def do_full_pipeline_benchmarks(
    pipeline: PipelineBenchmarkType, repeats: int = 1
) -> dict[str, list[float]]:
    """
    Do the projection benchmarks
    """
    code_to_benchmark = get_code_for_pipeline_benchmark(pipeline)
    benchmark_results = exec_pipeline_benchmarks_empty_inspection(
        code_to_benchmark, repeats
    )
    return benchmark_results


def get_code_for_op_benchmark(
    data_frame_rows: int, operator_type: OperatorBenchmarkType
) -> CodeToBenchmark:
    """
    ...
    if operator_type == OperatorBenchmarkType.PROJECTION:
        ...
        benchmark_setup_func_str = "get_single_df_creation_str({})".format(
            data_frame_rows
        )
        ...
        benchmark_setup_func_str = "get_multiple_dfs_creation_str({})".format(
            data_frame_rows
        )
        ...
        benchmark_setup_func_str = "get_np_cat_array_str({})".format(
            data_frame_rows
        )
        ...
        benchmark_setup_func_str = "get_np_num_array_str({})".format(
            data_frame_rows
        )
        ...
        benchmark_setup_func_str = "get_estimator_train_data_str({})".format(
            data_frame_rows
        )
        benchmark_exec_func_str = "get_decision_tree_str()"
    else:
        assert False
    code_to_benchmark = CodeToBenchmark(
        benchmark_setup,
        benchmark_exec,
        benchmark_setup_func_str,
        benchmark_exec_func_str,
    )
    return code_to_benchmark


def get_code_for_pipeline_benchmark(
    pipeline_type: PipelineBenchmarkType,
) -> CodeToBenchmark:
    """
    Get the code to benchmark for the operator benchmark type
    """
    if pipeline_type == PipelineBenchmarkType.HEALTHCARE:
        ...
        assert False
    code_to_benchmark = CodeToBenchmark(
        "pass", benchmark_exec, benchmark_setup_func_str, ""
    )
    return code_to_benchmark


def exec_benchmarks_empty_inspection(
    code_to_benchmark: CodeToBenchmark, repeats: int
) -> dict[str, list[float]]:
    """
    Benchmark some code without mlinspect and with mlinspect with varying numbers of inspections
    """
    benchmark_results = {
        "no mlinspect": timeit.repeat(
            stmt=code_to_benchmark.benchmark_exec,
            setup=code_to_benchmark.benchmark_setup,
            repeat=repeats,
            number=1,
        ),
        "no inspection": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[]",
            repeats,
        ),
        "one inspection": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0)]",
            repeats,
        ),
        "two inspections": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0), EmptyInspection(1)]",
            repeats,
        ),
        "three inspections": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0), "
...
def exec_benchmarks_nonempty_inspection(
    code_to_benchmark: CodeToBenchmark, repeats: int
) -> dict[str, list[float]]:
    """
    Benchmark some code without mlinspect and with mlinspect with varying numbers of inspections
    """
    benchmark_results = {
        "empty inspection": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0)]",
            repeats,
        ...
        "MaterializeFirstOutputRows(10)": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[MaterializeFirstOutputRows(10)]",
            repeats,
        ),
        "RowLineage(10)": benchmark_code_str_with_inspections(
            code_to_benchmark.benchmark_exec_func_str,
            code_to_benchmark.benchmark_setup_func_str,
            "[RowLineage(10)]",
            repeats,
...
def exec_pipeline_benchmarks_empty_inspection(
    code_to_benchmark: CodeToBenchmark, repeats: int
) -> dict[str, list[float]]:
    """
    Benchmark some code without mlinspect and with mlinspect with varying numbers of inspections
    ...
    benchmark_results = {
        ...
        "no inspection": benchmark_pipeline_code_str_with_inspections(
            code_to_benchmark.benchmark_setup_func_str, "[]", repeats
        ),
        "one inspection": benchmark_pipeline_code_str_with_inspections(
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0)]",
            repeats,
        ),
        "two inspections": benchmark_pipeline_code_str_with_inspections(
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0), EmptyInspection(1)]",
            repeats,
        ),
        "three inspections": benchmark_pipeline_code_str_with_inspections(
            code_to_benchmark.benchmark_setup_func_str,
            "[EmptyInspection(0), "
            + "EmptyInspection(1), EmptyInspection(2)]",
            repeats,
...
def benchmark_code_str_with_inspections(
    benchmark_str: str, setup_str: str, inspections_str: str, repeats: int
) -> list[float]:
    """
    ...
    benchmark_result_one_inspection = timeit.repeat(
        stmt=benchmark, setup=setup, repeat=repeats, number=1
    )
    return benchmark_result_one_inspection


def benchmark_pipeline_code_str_with_inspections(
    setup_str: str, inspections_str: str, repeats: int
) -> list[float]:
    """
    Execute one single benchmark
    """
...
def prepare_benchmark_exec(
    benchmark_str: str, setup_str: str, inspections: str
) -> str:
    """
    ...
    test_code_benchmark = {}
    """.format(
            setup_str, inspections, benchmark_str
        )
    )
    return setup


def trigger_benchmark_exec(inspections_str: str) -> str:
    """
...
def prepare_pipeline_benchmark_exec(test_code: str) -> str:
    """
    Get the benchmark str for timeit
    """
    benchmark = cleandoc(
...
def trigger_pipeline_benchmark_exec(inspections_str: str) -> str:
    """
    Get the benchmark str for timeit
    """
    benchmark = cleandoc(
...
def get_single_df_creation_str(data_frame_rows: int) -> str:
    """
...
def get_test_projection_str() -> str:
    """
...
def get_test_selection_str() -> str:
    """
    Get a pandas projection code str
    """
    test_code = cleandoc(
...
def get_multiple_dfs_creation_str(data_frame_rows: int) -> str:
    """
    Get a complete code str that creates a DF with random value
    """
    ...
    test_code = cleandoc(
        ...
        """.format(
            sizes_before_join=sizes_before_join,
            start_with_offset=start_with_offset,
...
def get_np_cat_array_str(data_frame_rows: int) -> str:
    """
...
def get_test_one_hot_encoder_str() -> str:
    """
    Get a pandas projection code str
    """
    test_code = cleandoc(
...
def get_np_num_array_str(data_frame_rows: int) -> str:
    """
    Get a complete code str that creates a np array with random values
    """
    test_code = cleandoc(
...
def get_test_standard_scaler_str() -> str:
    """
    Get a pandas projection code str
    """
    test_code = cleandoc(
...
def get_estimator_train_data_str(data_frame_rows: int) -> str:
    """
    Get a complete code str that creates a np array with random values
    """
    test_code = cleandoc(
...
def get_adult_simple_py_str() -> str:
    """
...
def get_adult_complex_py_str() -> str:
    """
    Get the code str for the adult_easy pipeline
    """
