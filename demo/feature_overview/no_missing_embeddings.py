...
from demo.feature_overview.missing_embeddings import (
    MissingEmbeddings,
    MissingEmbeddingsInfo,
)

from mlinspect import DagNode
from mlinspect.checks import Check, CheckResult, CheckStatus
...
class NoMissingEmbeddings(Check):
    ...
    def __init__(self, example_threshold: int = 10):
    ...
    def evaluate(self, inspection_result: InspectionResult) -> CheckResult:
        ...
        for (
            dag_node,
            dag_node_inspection_result,
        ) in inspection_result.dag_node_to_inspection_results.items():
            if (
                MissingEmbeddings(self.example_threshold)
                in dag_node_inspection_result
            ):
                missing_embedding_info = dag_node_inspection_result[
                    MissingEmbeddings(self.example_threshold)
                ]
                assert missing_embedding_info is None or isinstance(
                    missing_embedding_info, MissingEmbeddingsInfo
                )
                if (
                    missing_embedding_info is not None
                    and missing_embedding_info.missing_embedding_count > 0
                ):
                    dag_node_to_missing_embeddings[dag_node] = (
                        missing_embedding_info
                    )
        if dag_node_to_missing_embeddings:
            description = "Missing embeddings were found!"
            result = NoMissingEmbeddingsResult(
                self,
                CheckStatus.FAILURE,
                description,
                dag_node_to_missing_embeddings,
            ...
            result = NoMissingEmbeddingsResult(
                self, CheckStatus.SUCCESS, None, collections.OrderedDict()
            )
        return result
