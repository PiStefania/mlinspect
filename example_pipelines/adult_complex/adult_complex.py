...
train_labels = preprocessing.label_binarize(
    train_data["income-per-year"], classes=[">50K", "<=50K"]
)
test_labels = preprocessing.label_binarize(
    test_data["income-per-year"], classes=[">50K", "<=50K"]
)
...
nested_feature_transformation = ColumnTransformer(
    transformers=[
        (
            "categorical",
            nested_categorical_feature_transformation,
            ["education", "workclass"],
        ),
