...
class PipelineExecutor:
    ...
    source_code = None
    script_scope: dict = {}
    lineno_next_call_or_subscript: int = -1
    col_offset_next_call_or_subscript: int = -1
    end_lineno_next_call_or_subscript: int = -1
    end_col_offset_next_call_or_subscript: int = -1
    next_op_id: int = 0
    next_missing_op_id: int = -1
    track_code_references: bool = True
    op_id_to_dag_node: dict = {}
    inspection_results: InspectionResult = InspectionResult(
        networkx.DiGraph(), dict()
    )
    inspections: List[Inspection] = []
    custom_monkey_patching: List[Any] = []
    ...
    def run(
        ...
        *,
        ...
        python_path: str = "",
        python_code: str = "",
        inspections: Iterable[Inspection] | None = None,
        checks: Iterable[Check] | None = None,
        reset_state: bool = True,
        track_code_references: bool = True,
        custom_monkey_patching: List[Any] | None = None
    ) -> InspectorResult:
        """
        ...
        check_to_results = dict(
            (check, check.evaluate(self.inspection_results))
            for check in checks
        )
        return InspectorResult(
            self.inspection_results.dag,
            self.inspection_results.dag_node_to_inspection_results,
            check_to_results,
        )

    def run_inspections(
        self, notebook_path: str, python_code: Any, python_path: str
    ) -> None:
        """
        Instrument and execute the pipeline
        """
        # pylint: disable=no-self-use, too-many-locals
        self.source_code, self.source_code_path = self.load_source_code(
            notebook_path, python_path, python_code
        )
        parsed_ast = ast.parse(self.source_code)
        parsed_modified_ast = self.instrument_pipeline(
            parsed_ast, self.track_code_references
        )
    ...
    def get_next_missing_op_id(self) -> int:
        """
    ...
    def instrument_pipeline(
        parsed_ast: Any, track_code_references: bool
    ) -> Any:
        """
        ...
        func_import_node = ast.ImportFrom(
            module="mlinspect.instrumentation._pipeline_executor",
            names=[
                ast.alias(name="set_code_reference_call", asname=None),
        ...
        # monkey_patch()
        inspect_import_node = ast.Expr(
            value=ast.Call(
                func=ast.Name(id="monkey_patch", ctx=ast.Load()),
                args=[],
                keywords=[],
            )
        )
        ...
        inspect_import_node = ast.Expr(
            value=ast.Call(
                func=ast.Name(id="undo_monkey_patch", ctx=ast.Load()),
                args=[],
                keywords=[],
            )
        )
    ...
    def load_source_code(
        notebook_path: str, python_path: str, python_code: Any
    ) -> tuple[str, str]:
        """
...
def set_code_reference_subscript(
    lineno: int,
    col_offset: int,
    end_lineno: int,
    end_col_offset: int,
...
def get_monkey_patching_patch_sources() -> list:
    """
