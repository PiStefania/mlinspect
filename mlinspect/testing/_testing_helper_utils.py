...
from mlinspect.inspections._materialize_first_output_rows import (
...
from mlinspect.testing._random_annotation_testing_inspection import (
    RandomAnnotationTestingInspection,
)
...
def get_expected_dag_adult_easy(
    caller_filename: str,
    line_offset: int = 0,
    with_code_references: bool = True,
) -> DiGraph:
    """
    ...
    expected_data_source = DagNode(
        ...
        OptionalCodeInfo(
            CodeReference(19 + line_offset, 11, 19 + line_offset, 62),
    ...
    expected_select = DagNode(
        1,
        BasicCodeLocation(caller_filename, 21 + line_offset),
        OperatorContext(
            OperatorType.SELECTION, FunctionInfo("pandas.core.frame", "dropna")
        ),
        DagNodeDetails(
            "dropna",
            [
        ...
        OptionalCodeInfo(
            CodeReference(21 + line_offset, 7, 21 + line_offset, 24),
            "raw_data.dropna()",
        ),
    )
    ...
    expected_pipeline_project_one = DagNode(
        4,
        BasicCodeLocation(caller_filename, 27 + line_offset),
        OperatorContext(
        ...
        DagNodeDetails(
            "to ['education', 'workclass']", ["education", "workclass"]
        ),
        OptionalCodeInfo(
            CodeReference(27 + line_offset, 25, 36 + line_offset, 1),
            pipeline_str,
        ),
    )
    ...
    expected_pipeline_project_two = DagNode(
        ...
        DagNodeDetails(
            "to ['age', 'hours-per-week']", ["age", "hours-per-week"]
        ),
        OptionalCodeInfo(
            CodeReference(27 + line_offset, 25, 36 + line_offset, 1),
            pipeline_str,
    ...
    expected_pipeline_transformer_one = DagNode(
        5,
        BasicCodeLocation(caller_filename, 31 + line_offset),
        OperatorContext(
        ...
        OptionalCodeInfo(
            CodeReference(31 + line_offset, 12, 31 + line_offset, 64),
            'preprocessing.OneHotEncoder(handle_unknown="ignore")',
        ),
    )
    expected_pipeline_transformer_two = DagNode(
        7,
        BasicCodeLocation(caller_filename, 34 + line_offset),
        OperatorContext(
            OperatorType.TRANSFORMER,
        ...
        OptionalCodeInfo(
            CodeReference(34 + line_offset, 20, 34 + line_offset, 50),
            "preprocessing.StandardScaler()",
        ),
    )
    expected_graph.add_edge(
        expected_pipeline_project_one, expected_pipeline_transformer_one
    )
    expected_graph.add_edge(
        expected_pipeline_project_two, expected_pipeline_transformer_two
    )
    ...
    expected_graph.add_edge(
        expected_pipeline_transformer_one, expected_pipeline_concatenation
    )
    expected_graph.add_edge(
        expected_pipeline_transformer_two, expected_pipeline_concatenation
    )

    expected_train_data = DagNode(
        9,
        BasicCodeLocation(caller_filename, 42 + line_offset),
        OperatorContext(
        ...
        DagNodeDetails(None, ["array"]),
        OptionalCodeInfo(
            CodeReference(42 + line_offset, 23, 42 + line_offset, 52),
            "tree.DecisionTreeClassifier()",
        ),
    )
    expected_graph.add_edge(
        expected_pipeline_concatenation, expected_train_data
    )

    expected_project = DagNode(
        2,
        BasicCodeLocation(caller_filename, 24 + line_offset),
        OperatorContext(
            OperatorType.PROJECTION,
        ...
        OptionalCodeInfo(
            CodeReference(24 + line_offset, 4, 24 + line_offset, 27),
    ...
    expected_project_modify = DagNode(
        3,
        BasicCodeLocation(caller_filename, 23 + line_offset),
        OperatorContext(
        ...
        OptionalCodeInfo(
            CodeReference(23 + line_offset, 9, 25 + line_offset, 1),
...
def get_pandas_read_csv_and_dropna_code() -> str:
    """
...
def run_random_annotation_testing_analyzer(code: str) -> dict:
    """
    ...
    for dag_node, inspection_result in inspection_results.items():
        ...
        dag_node_to_random_inspection[dag_node] = inspection_result[
            RandomAnnotationTestingInspection(10)
        ]
    return dag_node_to_random_inspection


def run_row_index_annotation_testing_analyzer(code: str) -> dict:
    """
    An utility function to test backends
    """
    result = (
...
def run_multiple_test_analyzers(code: str) -> tuple:
    """
    An utility function to test backends.
    Also useful to debug annotation propagation.
    """
    analyzers = [
        RandomAnnotationTestingInspection(2),
        MaterializeFirstOutputRows(5),
        RowLineage(2),
    ]
    result = (
        PipelineInspector.on_pipeline_from_string(code)
...
def run_and_assert_all_op_outputs_inspected(
    py_file_path: str,
    sensitive_columns: List[str],
    dag_png_path: str,
    custom_monkey_patching: List | None = None,
) -> DiGraph:
    """
    ...
    inspector_result = (
        PipelineInspector.on_pipeline_from_py_file(py_file_path)
        ...
        .add_required_inspection(
            IntersectionalHistogramForColumns(sensitive_columns)
        )
    ...
    for (
        dag_node,
        inspection_result,
    ) in inspector_result.dag_node_to_inspection_results.items():
        ...
        if dag_node.operator_info.operator is not OperatorType.ESTIMATOR:
            ...
            assert (
                inspection_result[HistogramForColumns(sensitive_columns)]
                is not None
            )
            assert (
                inspection_result[ColumnPropagation(sensitive_columns, 5)]
                is not None
            )
            assert (
                inspection_result[
            ...
            assert (
                inspection_result[CompletenessOfColumns(sensitive_columns)]
                is not None
            )
            assert (
                inspection_result[CountDistinctOfColumns(sensitive_columns)]
                is not None
            )
        else:
...
def get_test_code_with_function_def_and_for_loop() -> str:
    """
