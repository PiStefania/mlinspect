...
class ColumnPropagation(Inspection):
    ...
    def __init__(self, sensitive_columns: Any, row_count: int) -> None:
        self.row_count = row_count
        self._operator_type = None
        self.sensitive_columns = sensitive_columns
        self._op_output: List[tuple] | None = None
        self._op_annotations: List[List[Any]] | None = None
        self._output_columns: List[str] | None = None
        self._is_sink = False

    @property
    def inspection_id(self) -> tuple:
    ...
    def visit_operator(
        ...
        if isinstance(inspection_input, InspectionInputUnaryOperator):
            ...
            for row_data_source in inspection_input.row_iterator:
                ...
                if current_count < self.row_count:
                    operator_output.append(row_data_source.output)
                    operator_annotations.append(column_values)
                yield column_values
                    ...
                    if sensitive_columns_present[check_index]:
                        ...
                        if sensitive_columns_present[check_index]:
                            ...
                            column_value_candidates = [
                                annotation[check_index]
                                for annotation in row_nary.annotation
                        ...
                        column_values.append(column_value)
                if current_count < self.row_count:
                    operator_output.append(row_nary.output)
                    operator_annotations.append(column_values)
                yield column_values
            ...
            for row_sink in inspection_input.row_iterator:
                ...
                for check_index, _ in enumerate(self.sensitive_columns):
                    column_value_candidates = [
                        annotation[check_index]
                        for annotation in row_sink.annotation
                        if annotation[check_index] is not None
                    ]
                    if len(column_value_candidates) >= 1:
                        column_value = column_value_candidates[0]
    ...
    def get_operator_annotation_after_visit(self) -> Any:
        ...
        new_sensitive_column_names = [
            f"mlinspect_{column_name}"
            for column_name in self.sensitive_columns
        ]
        if not self._is_sink:
            original_output_df = pandas.DataFrame(
                self._op_output, columns=self._output_columns
            )
            output_annotations = pandas.DataFrame(
                self._op_annotations, columns=new_sensitive_column_names
            )
            result = pandas.concat(
                [original_output_df, output_annotations], axis=1
            )
        else:
