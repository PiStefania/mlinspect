...
class BiasDistributionChange:
    ...
    def __eq__(self, other: object) -> bool:
        return (
            ...
            and self.acceptable_change == other.acceptable_change
            and self.min_relative_ratio_change
            == other.min_relative_ratio_change
...
class NoBiasIntroducedFor(Check):
    ...
    def __init__(
        ...
        min_allowed_relative_ratio_change: float = -0.3,
    ):
        self.sensitive_columns = sensitive_columns
        self.min_allowed_relative_ratio_change = (
            min_allowed_relative_ratio_change
        )
    ...
    def evaluate(self, inspection_result: InspectionResult) -> CheckResult:
        ...
        bias_distribution_change: dict[
            DagNode, dict[str, BiasDistributionChange]
        ...
        for node in relevant_nodes:
            ...
            for column in self.sensitive_columns:
                ...
                if not column_result.acceptable_change:
                    issue = (
                        "A {} causes a min_relative_ratio_change of '{}' by {}, a value below the "
                        "configured minimum threshold {}!".format(
                            node.operator_info.operator.value,
                            column,
                            column_result.min_relative_ratio_change,
                            self.min_allowed_relative_ratio_change,
                        )
                    )
        ...
        if issue_list:
            ...
            description = None
        return NoBiasIntroducedForResult(
    ...
    def get_histograms_for_node_and_column(
        ...
        joined_df["ratio_before"] = (
            joined_df["count_before"] / joined_df["count_before"].sum()
        )
        joined_df["ratio_after"] = (
            joined_df["count_after"] / joined_df["count_after"].sum()
        )
        absolute_ratio_change = (
            joined_df["ratio_after"] - joined_df["ratio_before"]
        )
        joined_df["relative_ratio_change"] = (
            absolute_ratio_change / joined_df["ratio_before"]
        )
        ...
        min_relative_ratio_change = joined_df[not_nan][
            "relative_ratio_change"
        ].min()

        all_changes_acceptable = (
            min_relative_ratio_change >= self.min_allowed_relative_ratio_change
        )
        return BiasDistributionChange(
            node, all_changes_acceptable, min_relative_ratio_change, joined_df
        )
    ...
    def plot_distribution_change_histograms(
        distribution_change: BiasDistributionChange,
        filename: str = "",
    ...
    @staticmethod
    def get_distribution_changes_overview_as_df(
        no_bias_check_result: NoBiasIntroducedForResult,
    ) -> DataFrame:
        """
        ...
        sensitive_column_names = [
            "'{}' distribution change below the configured minimum test threshold".format(
                name
            )
            for name in sensitive_column_names
        ]
        sensitive_columns: List[Any] = []
        for _ in range(len(sensitive_column_names)):
            sensitive_columns.append([])
        for (
            dag_node,
            distribution_change,
        ) in no_bias_check_result.bias_distribution_change.items():
            ...
            for index, change_info in enumerate(distribution_change.values()):
                sensitive_columns[index].append(
                    not change_info.acceptable_change
                )
        return DataFrame(
            zip(
                operator_types,
