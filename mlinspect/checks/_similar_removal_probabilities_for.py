...
from mlinspect.inspections._inspection_input import FunctionInfo, OperatorType
...
class RemovalProbabilities:
    ...
    def __eq__(self, other: object) -> bool:
        return (
            ...
            and self.dag_node == other.dag_node
            and self.max_probability_difference
            == other.max_probability_difference
            and self.before_and_after_df.equals(other.before_and_after_df)
        )
...
class SimilarRemovalProbabilitiesFor(Check):
    ...
    def __init__(
        ...
        max_allowed_probability_difference: float = 2.0,
    ) -> None:
        self.sensitive_columns = sensitive_columns
        self.max_allowed_probability_difference = (
            max_allowed_probability_difference
        )
    ...
    def evaluate(self, inspection_result: InspectionResult) -> CheckResult:
        """Evaluate the check"""
        histograms, relevant_nodes = self.get_relevant_nodes_and_histograms(
            inspection_result
        )
        dag = inspection_result.dag
        check_status = CheckStatus.SUCCESS
        bias_distribution_change: dict[
            DagNode, dict[str, RemovalProbabilities]
        ...
        for node in relevant_nodes:
            ...
            for column in self.sensitive_columns:
                ...
                if not column_result.acceptable_probability_difference:
                    issue = (
                        "A {} causes a max_probability_difference of '{}' by {}, a value above the "
                        "configured maximum threshold {}!".format(
                            node.operator_info.operator.value,
                            column,
                            column_result.max_probability_difference,
                            self.max_allowed_probability_difference,
                        )
                    )
        ...
        if issue_list:
            ...
            description = None
        return SimilarRemovalProbabilitiesForResult(
            self, check_status, description, bias_distribution_change
        )

    def get_relevant_nodes_and_histograms(
        ...
        for (
            ...
            histograms[dag_node] = inspection_results[
                HistogramForColumns(self.sensitive_columns)
            ]
        relevant_nodes = [
            node
            for node in dag.nodes
            if node.operator_info.operator
            in {OperatorType.JOIN, OperatorType.SELECTION}
            or (
                node.operator_info.function_info
                == FunctionInfo("sklearn.impute._base", "SimpleImputer")
                and set(node.details.columns).intersection(
    ...
    def get_histograms_for_node_and_column(
        self,
        column: str,
        histograms: dict,
        node: DagNode,
        ...
        after_df = DataFrame(
            after_map.items(),
            columns=["sensitive_column_value", "count_after"],
        )
        ...
        before_df = DataFrame(
            before_map.items(),
            columns=["sensitive_column_value", "count_before"],
        )

        joined_df = before_df.merge(
            after_df, on="sensitive_column_value", how="outer"
        )
        joined_df = joined_df.sort_values(
            by=["sensitive_column_value"]
        ).reset_index(drop=True)
        joined_df["count_before"] = joined_df["count_before"].fillna(
            0, downcast="infer"
        )
        joined_df["count_after"] = joined_df["count_after"].fillna(
            0, downcast="infer"
        )

        # Probability of removal
        joined_df["removed_records"] = (
            joined_df["count_before"] - joined_df["count_after"]
        )
        joined_df["removal_probability"] = (
            joined_df["removed_records"] / joined_df["count_before"]
        )
        ...
        removal_probability_min = joined_df[non_zero_probabilities][
            "removal_probability"
        ].min()
        joined_df["normalized_removal_probability"] = (
            joined_df["removal_probability"] / removal_probability_min
        )
        joined_df.loc[
            joined_df["removed_records"] < 0, "removal_probability"
        ] = 0
        joined_df.loc[
            joined_df["removed_records"] < 0, "normalized_removal_probability"
        ] = 0
        ...
        not_nan_normalized_removal_probabilities = joined_df[
            "normalized_removal_probability"
        ].notnull()
        relevant_normalized_probabs = (
            not_nan_columns & not_nan_normalized_removal_probabilities
        )
        max_normalized_removal_probab = joined_df[relevant_normalized_probabs][
            "normalized_removal_probability"
        ].max()
        acceptable_probability_difference = (
            max_normalized_removal_probab
            <= self.max_allowed_probability_difference
        )

        return RemovalProbabilities(
            node,
            acceptable_probability_difference,
            max_normalized_removal_probab,
            joined_df,
    ...
    @staticmethod
    def plot_distribution_change_histograms(
        distribution_change: RemovalProbabilities,
        filename: str = "",
        ...
        keys = distribution_change.before_and_after_df[
    ...
    @staticmethod
    def plot_removal_probability_histograms(
        distribution_change: RemovalProbabilities,
        filename: str = "",
        save_to_file: bool = False,
    ) -> None:
        ...
        keys = [str(key) for key in keys]  # Necessary because of null values
        removal_probabilities = distribution_change.before_and_after_df[
            "removal_probability"
        ]

        pyplot.bar(keys, removal_probabilities)
        pyplot.gca().set_title(
            "removal probability per member of sensitive group"
        )
        pyplot.xticks(
            rotation=45,
            horizontalalignment="right",
    ...
    @staticmethod
    def get_removal_probabilities_overview_as_df(
        removal_probab_check_result: SimilarRemovalProbabilitiesForResult,
    ) -> DataFrame:
        """
        ...
        operator_types = []
        code_references: List[Any] = []
        function_infos = []
        code_snippets = []
        descriptions = []
        assert isinstance(
            removal_probab_check_result.check, SimilarRemovalProbabilitiesFor
        )
        sensitive_column_names = []
        for name in removal_probab_check_result.check.sensitive_columns:
            removal_probability_column_name = (
                "'{}' probability difference below the configured maximum test "
        ...
        for (
            dag_node,
            removal_probability,
        ) in removal_probab_check_result.removal_probability_change.items():
            ...
            if dag_node.optional_code_info is not None:
                ...
                code_snippets.append(
                    "You can enable code reference tracking for more details."
                )
            ...
            for index, change_info in enumerate(removal_probability.values()):
                sensitive_columns[index].append(
                    not change_info.acceptable_probability_difference
                )
        return DataFrame(
            zip(
                operator_types,
