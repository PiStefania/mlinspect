...
class NoIllegalFeatures(Check):
    ...
    def __init__(
        self, additional_illegal_feature_names: List | None = None
    ) -> None:
        if additional_illegal_feature_names is None:
            additional_illegal_feature_names = []
        self.additional_illegal_feature_names: List = (
            additional_illegal_feature_names
        )
    ...
    def evaluate(self, inspection_result: InspectionResult) -> CheckResult:
        ...
        forbidden_columns = {
            *ILLEGAL_FEATURES,
            *self.additional_illegal_feature_names,
        }
        used_illegal_columns = list(
            set(used_columns).intersection(forbidden_columns)
        )
        if used_illegal_columns:
            description = "Used illegal columns: {}".format(
                used_illegal_columns
            )
            result = NoIllegalFeaturesResult(
                self, CheckStatus.FAILURE, description, used_illegal_columns
            )
        else:
            result = NoIllegalFeaturesResult(
                self, CheckStatus.SUCCESS, None, []
            )
        return result

    def get_used_columns(
        self, dag: Any, current_node: DagNode
    ) -> list[str] | list:
        """
