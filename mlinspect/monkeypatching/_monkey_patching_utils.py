...
def execute_patched_func(
    original_func: Any,
    execute_inspections_func: Any,
    *args: Any,
    **kwargs: Any
) -> Any:
    """
    ...
    caller_filename = sys._getframe(
        2
    ).f_code.co_filename  # pylint: disable=protected-access
    ...
    if caller_filename != singleton.source_code_path:
        ...
        result = execute_inspections_func(
            op_id,
            caller_filename,
            caller_lineno,
            caller_code_reference,
            caller_source_code,
        )
    else:
        op_id = singleton.get_next_op_id()
        caller_lineno = sys._getframe(
            2
        ).f_lineno  # pylint: disable=protected-access
        result = execute_inspections_func(
            op_id, caller_filename, caller_lineno, None, None
        )
    return result


def execute_patched_func_no_op_id(
    original_func: Any,
    execute_inspections_func: Any,
    *args: Any,
    **kwargs: Any
    ...
    if caller_filename != singleton.source_code_path:
        ...
        result = execute_inspections_func(
            -1, caller_filename, caller_lineno, None, None
        )
    return result


def execute_patched_func_indirect_allowed(
    execute_inspections_func: Any,
) -> Any:
    """
    Detects whether the function call comes directly from user code and decides whether to execute the original
    ...
    frame: Any = sys._getframe(2)  # pylint: disable=protected-access
...
def get_input_info(
    ...
    if hasattr(df_object, "_mlinspect_annotation"):
        ...
        input_info = InputInfo(
            input_dag_node, AnnotatedDfObject(df_object, annotation_df)
        )
    else:
        operator_context = OperatorContext(
            OperatorType.DATA_SOURCE, function_info
        )
        backend_result = execute_inspection_visits_data_source(
            operator_context, df_object, {}
        )
        if optional_code_reference:
            ...
            code_reference = ""
        description = (
            "Warning! Operator {}:{} {} encountered a DataFrame resulting from an operation "
            "without mlinspect support!".format(
                caller_filename, lineno, code_reference
            )
        )
        missing_op_id = singleton.get_next_missing_op_id()
        input_dag_node = DagNode(
            missing_op_id,
...
def get_column_names(df_object: Any) -> list[str]:
    ...
    if isinstance(df_object, DataFrame):
        ...
        raise NotImplementedError(
            "TODO: Type: '{}' still is not supported!".format(type(df_object))
        )
    return columns
...
def add_dag_node(
    ...
    if annotated_df.result_data is not None:
        ...
        if annotated_df.result_annotation is not None:
            ...
            annotated_df.result_data._mlinspect_annotation = (
                annotated_df.result_annotation
            )
    if dag_node_parents:
    ...
    if annotated_df is not None:
        singleton.inspection_results.dag_node_to_inspection_results[
            dag_node
        ] = backend_result.dag_node_annotation


def get_dag_node_for_id(dag_node_id: int) -> DagNode | Any:
    """
...
def get_optional_code_info_or_none(
    optional_code_reference: CodeReference | None,
    optional_source_code: str | None,
) -> OptionalCodeInfo | None:
    """
    ...
    if singleton.track_code_references:
        ...
        code_info_or_none = OptionalCodeInfo(
...
def add_train_label_node(
    estimator: Any, train_label_arg: Any, function_info: FunctionInfo
) -> tuple[BackendResult, DagNode, Any]:
    """Add a Train Data DAG Node for a estimator.fit call"""
    operator_context = OperatorContext(
        OperatorType.TRAIN_LABELS, function_info
    )
    input_info_train_labels = get_input_info(
        train_label_arg,
        estimator.mlinspect_caller_filename,
        estimator.mlinspect_lineno,
        function_info,
    ...
    train_labels_dag_node = DagNode(
        train_label_op_id,
        BasicCodeLocation(
            estimator.mlinspect_caller_filename, estimator.mlinspect_lineno
        ),
        operator_context,
    ...
    input_infos = SklearnBackend().before_call(
        operator_context, [input_info_train_labels.annotated_dfobject]
    )
    label_backend_result = SklearnBackend().after_call(
        operator_context, input_infos, train_label_arg
    )
    add_dag_node(
...
def add_train_data_node(
    estimator: Any, train_data_arg: Any, function_info: FunctionInfo
) -> tuple[BackendResult, DagNode, Any]:
    ...
    data_backend_result = SklearnBackend().after_call(
        operator_context, input_infos, train_data_arg
    )
    add_dag_node(
        train_data_dag_node,
...
def add_test_data_dag_node(
    ...
    input_info_test_data = get_input_info(
        test_data_arg,
        caller_filename,
        lineno,
        function_info,
    ...
    input_infos = SklearnBackend().before_call(
        operator_context, [input_info_test_data.annotated_dfobject]
    )
    data_backend_result = SklearnBackend().after_call(
        operator_context, input_infos, test_data_arg
    )
    add_dag_node(
        test_data_dag_node,
...
def add_test_label_node(
    ...
    input_info_test_labels = get_input_info(
        test_label_arg,
        caller_filename,
        lineno,
        function_info,
    ...
    label_backend_result = SklearnBackend().after_call(
        operator_context, input_infos, test_label_arg
    )
    add_dag_node(
        test_labels_dag_node,
