...
from .. import FunctionInfo, OperatorContext, OperatorType
...
from ._iter_creation import (
    iter_input_annotation_output_join,
    iter_input_annotation_output_map,
    iter_input_annotation_output_resampled,
    iter_input_data_source,
)
...
class PandasBackend(Backend):
    ...
    def before_call(
        ...
        elif operator_context.function_info == FunctionInfo(
            "pandas.core.frame", "merge"
        ):
            first_pandas_df = input_infos[0].result_data
            assert isinstance(first_pandas_df, pandas.DataFrame)
            first_pandas_df["mlinspect_index_x"] = range(
                0, len(first_pandas_df)
            )
            second_pandas_df = input_infos[1].result_data
            assert isinstance(second_pandas_df, pandas.DataFrame)
            second_pandas_df["mlinspect_index_y"] = range(
                0, len(second_pandas_df)
            )
        return input_infos
    ...
    def after_call(
        ...
        if operator_context.operator == OperatorType.DATA_SOURCE:
            ...
            reset_index_return_value = execute_inspection_visits_data_source(
                operator_context, df_reset_index, non_data_function_args
            )
            annotated_result_object = AnnotatedDfObject(
                return_value,
                reset_index_return_value.annotated_dfobject.result_annotation,
            )
            return_value_be = BackendResult(
                annotated_result_object,
                reset_index_return_value.dag_node_annotation,
            )
            ...
            return_value_be = execute_inspection_visits_join(
                operator_context,
                input_infos[0].result_data,
                input_infos[0].result_annotation,
            ...
            input_infos[0].result_data.drop(
                "mlinspect_index_x", axis=1, inplace=True
            )
            input_infos[1].result_data.drop(
                "mlinspect_index_y", axis=1, inplace=True
            )

        else:
            raise NotImplementedError(
                "PandasBackend doesn't know any operations of type '{}' yet!".format(
...
def execute_inspection_visits_data_source(
    operator_context: OperatorContext,
    return_value: Any,
    ...
    iterators_for_inspections = iter_input_data_source(
        inspection_count,
        return_value,
        operator_context,
        non_data_function_args,
    )
    return_value_be = execute_visits_and_store_results(
        iterators_for_inspections, return_value
    )
    return return_value_be


def execute_inspection_visits_unary_operator(
    operator_context: OperatorContext,
    input_data: Any,
    ...
    inspection_count = len(singleton.inspections)
    if resampled:
        iterators_for_inspections = iter_input_annotation_output_resampled(
            inspection_count,
            input_data,
            input_annotations,
            return_value_df,
        ...
        iterators_for_inspections = iter_input_annotation_output_map(
            inspection_count,
            input_data,
            input_annotations,
            return_value_df,
    ...
    return_value = execute_visits_and_store_results(
        iterators_for_inspections, return_value_df
    )
    return return_value


def execute_inspection_visits_join(
    operator_context: OperatorContext,
    input_data_one: pandas.DataFrame,
    input_annotations_one: Any,
    input_data_two: pandas.DataFrame,
    input_annotations_two: Any,
    return_value_df: Any,
    non_data_function_args: Any,
) -> BackendResult:
    """Execute inspections when the current operator has one parent in the DAG"""
    ...
    inspection_count = len(singleton.inspections)
    iterators_for_inspections = iter_input_annotation_output_join(
        inspection_count,
        input_data_one,
...
def execute_visits_and_store_results(
    iterators_for_inspections: list[Any] | Dict, return_value: Any
) -> BackendResult:
    """
    ...
    for inspection_index, inspection in enumerate(singleton.inspections):
        ...
        annotation_iterator = inspection.visit_operator(
            iterator_for_inspection
        )
        annotation_iterators.append(annotation_iterator)
    return_value_be = store_inspection_outputs(
        annotation_iterators, return_value
    )
    return return_value_be
...
def store_inspection_outputs(
    annotation_iterators: Any, return_value: Any
) -> BackendResult:
    """
    ...
    """
    annotations_df = build_annotation_df_from_iters(
        singleton.inspections, annotation_iterators
    )
    inspection_outputs = {}
    for inspection in singleton.inspections:
        inspection_outputs[inspection] = (
            inspection.get_operator_annotation_after_visit()
        )
    new_return_value = create_wrapper_with_annotations(
        annotations_df, return_value
    )
