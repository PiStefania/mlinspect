...
from mlinspect.backends._backend_utils import (
    get_annotation_rows,
    get_df_row_iterator,
    get_iterator_for_type,
)
...
def iter_input_data_source(
    ...
    for inspection_index in range(inspection_count):
        ...
        inspection_iterator = InspectionInputDataSource(
            operator_context,
            output_columns,
            row_iterator,
...
def iter_input_annotation_output_map(
    inspection_count: int,
    input_data: Any,
    ...
    for inspection_index in range(inspection_count):
        ...
        output_iterator = duplicated_output_iterators[inspection_index]
        annotation_rows = get_annotation_rows(
            input_annotations, inspection_index
        )
        row_iterator = map(
            lambda input_tuple: InspectionRowUnaryOperator(*input_tuple),
            zip(input_iterator, annotation_rows, output_iterator),
        )
        inspection_iterator = InspectionInputUnaryOperator(
            operator_context,
            input_columns,
            output_columns,
            row_iterator,
...
def iter_input_annotation_output_resampled(
    inspection_count: int,
    input_data: Any,
    input_annotations: Any,
    output: Any,
    operator_context: OperatorContext,
    non_data_function_args: Any,
) -> list | list[InspectionInputUnaryOperator]:
    """
    ...
    data_before_with_annotations = pandas.concat(
        [input_data.reset_index(drop=True), input_annotations], axis=1
    )
    joined_df = output.merge(
        data_before_with_annotations,
        left_on="mlinspect_index",
    ...
    output_df_view = joined_df.iloc[
        :, 0 : column_index_output_end - 1
    ]  # -1 excludes the mlinspect_index
    output_df_view.columns = output.columns[
        0:-1
    ]  # -1 excludes the mlinspect_index
    output_columns, output_rows = get_df_row_iterator(output_df_view)
    duplicated_output_iterators = itertools.tee(output_rows, inspection_count)

    column_index_input_end = (
        column_index_output_end + len(input_data.columns) - 1
    )  # -1 excludes the mlinspect_index

    input_df_view = joined_df.iloc[
        :, column_index_output_end:column_index_input_end
    ]
    input_df_view.columns = input_data.columns[
        0:-1
    ]  # -1 excludes the mlinspect_index
    ...
    for inspection_index in range(inspection_count):
        ...
        column_annotation_current_inspection = (
            column_index_input_end + inspection_index
        )
        annotation_rows = get_annotation_rows(
            joined_df, column_annotation_current_inspection
        )
        row_iterator = map(
            lambda input_tuple: InspectionRowUnaryOperator(*input_tuple),
            zip(input_iterator, annotation_rows, output_iterator),
...
def iter_input_annotation_output_join(
    inspection_count: int,
    x_data: Any,
    x_annotations: Any,
    y_data: Any,
    ...
    operator_context: OperatorContext,
    non_data_function_args: Any,
) -> list | list[InspectionInputNAryOperator]:
    """
    ...
    x_before_with_annotations = pandas.concat(
        [x_data.reset_index(drop=True), x_annotations], axis=1
    )
    y_before_with_annotations = pandas.concat(
        [y_data.reset_index(drop=True), y_annotations], axis=1
    )
    df_x_output = output.merge(
        x_before_with_annotations,
        on="mlinspect_index_x",
        suffixes=["_output", "_x"],
    )
    df_x_output_y = df_x_output.merge(
        y_before_with_annotations,
        on="mlinspect_index_y",
        suffixes=["_x_output", "_y_output"],
    )

    df_x_output_y = df_x_output_y.drop(
        ["mlinspect_index_y", "mlinspect_index_x"], axis=1
    )
    ...
    column_index_output_end = (
        len(output.columns) - 2
    )  # -2 accounts for the index columns

    column_index_x_start = column_index_output_end
    column_index_x_end = (
        column_index_x_start + len(x_data.columns) - 1
    )  # -1 accounts for mlinspect_index_x
    column_index_y_start = column_index_x_end + inspection_count
    column_index_y_end = (
        column_index_y_start + len(y_data.columns) - 1
    )  # -1 accounts for mlinspect_index_y

    input_x_view = df_x_output_y.iloc[
        :, column_index_x_start:column_index_x_end
    ]
    input_x_view.columns = x_data.columns[
        0:-1
    ]  # -1 accounts for mlinspect_index_x
    input_y_view = df_x_output_y.iloc[
        :, column_index_y_start:column_index_y_end
    ]
    input_y_view.columns = y_data.columns[
        0:-1
    ]  # -1 accounts for mlinspect_index_y
    ...
    output_df_view = df_x_output_y.iloc[
        :, column_index_output_start:column_index_output_end
    ]
    output_df_view.columns = [
        column
        for column in output.columns
        if (column not in ("mlinspect_index_x", "mlinspect_index_y"))
    ]
    output_columns, output_rows = get_df_row_iterator(output_df_view)
    duplicated_output_iterators = itertools.tee(output_rows, inspection_count)
    ...
    for inspection_index in range(inspection_count):
        ...
        column_annotation_y_current_inspection = (
            column_index_y_end + inspection_index
        )
        column_annotation_x_current_inspection = (
            column_index_x_end + inspection_index
        )
        annotation_iterators = [
            get_annotation_rows(
                df_x_output_y, column_annotation_x_current_inspection
            ),
            get_annotation_rows(
                df_x_output_y, column_annotation_y_current_inspection
            ),
        ]
        ...
        row_iterator = map(
            lambda input_tuple: InspectionRowNAryOperator(*input_tuple),
            zip(input_iterator, annotation_rows, output_iterator),
        )
        inspection_iterator = InspectionInputNAryOperator(
            operator_context,
            inputs_columns,
            output_columns,
            row_iterator,
...
def iter_input_annotation_output_nary_op(
    inspection_count: int,
    annotated_inputs: List[AnnotatedDfObject],
    output_data: Any,
    operator_context: OperatorContext,
    non_data_function_args: Any,
) -> list | list[InspectionInputNAryOperator]:
    ...
    for annotated_input in annotated_inputs:
        column_info, row_iterator = get_iterator_for_type(
            annotated_input.result_data, True
        )
    ...
    for inspection_index in range(inspection_count):
        annotation_iterators = []
        for annotated_input in annotated_inputs:
            annotation_iterators.append(
                get_annotation_rows(
                    annotated_input.result_annotation, inspection_index
                )
            )
...
def iter_input_annotation_output_sink_op(
    inspection_count: int,
    data: Any,
    data_annotation: Any,
    target: Any,
    ...
    input_data_columns, input_data_iterators = get_iterator_for_type(
        data, False
    )
    input_target_columns, input_target_iterators = get_iterator_for_type(
        target, True
    )
    ...
    for inspection_index in range(inspection_count):
        input_iterator = duplicated_input_iterators[inspection_index]
        annotation_iterators = [
            get_annotation_rows(data_annotation, inspection_index),
            get_annotation_rows(target_annotation, inspection_index),
        ]
        annotation_rows = map(tuple, zip(*annotation_iterators))
        row_iterator = map(
            lambda input_tuple: InspectionRowSinkOperator(*input_tuple),
            zip(input_iterator, annotation_rows),
        )
        inspection_iterator = InspectionInputSinkOperator(
            operator_context,
            inputs_columns,
            row_iterator,
            non_data_function_args,
