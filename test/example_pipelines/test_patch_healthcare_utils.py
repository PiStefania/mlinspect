...
def test_my_word_to_vec_transformer() -> None:
    ...
    expected_data_source_two = DagNode(
        ...
        DagNodeDetails(None, ["A"]),
        OptionalCodeInfo(
            CodeReference(9, 10, 9, 67),
            "pd.DataFrame({'A': ['cat_a', 'cat_b', 'cat_a', 'cat_c']})",
        ),
    )
    expected_estimator_two = DagNode(
        3,
        BasicCodeLocation("<string-source>", 6),
        OperatorContext(
            OperatorType.TRANSFORMER,
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_estimator_two]
    )
    lineage_output = inspection_results_data_source[RowLineage(3)]
    expected_lineage_df = DataFrame(
        [
            [numpy.array([0.0, 0.0, 0.0]), {LineageId(2, 0)}],
...
def test_arg_capturing_my_word_to_vec_transformer() -> None:
    """
