...
from mlinspect import InspectorResult, OperatorType, PipelineInspector
from mlinspect.checks import (
    CheckStatus,
    NoBiasIntroducedFor,
    NoIllegalFeatures,
)
...
def test_inspector_adult_easy_py_pipeline() -> None:
    """
    ...
    compare(
        networkx.to_dict_of_dicts(extracted_dag),
        networkx.to_dict_of_dicts(expected_dag),
    )

    assert (
        HistogramForColumns(["race"])
        in list(inspector_result.dag_node_to_inspection_results.values())[0]
    )
    check_to_check_results = inspector_result.check_to_check_results
    assert (
        check_to_check_results[NoBiasIntroducedFor(["race"])].status
        == CheckStatus.SUCCESS
    )
    assert (
        check_to_check_results[NoIllegalFeatures()].status
        == CheckStatus.FAILURE
    )


def test_inspector_adult_easy_py_pipeline_without_inspections() -> None:
    """
    Tests whether the .py version of the inspector works
    """
    inspector_result = PipelineInspector.on_pipeline_from_py_file(
        ADULT_SIMPLE_PY
    ).execute()
    extracted_dag = inspector_result.dag
    expected_dag = get_expected_dag_adult_easy(ADULT_SIMPLE_PY)
...
def test_inspector_adult_easy_ipynb_pipeline() -> None:
    """
    Tests whether the .ipynb version of the inspector works
    """
    inspector_result = (
        PipelineInspector.on_pipeline_from_ipynb_file(ADULT_SIMPLE_IPYNB)
        .add_required_inspection(MaterializeFirstOutputRows(5))
        .add_check(NoBiasIntroducedFor(["race"]))
        .add_check(NoIllegalFeatures())
        .execute()
...
def test_inspector_adult_easy_str_pipeline() -> None:
    """
    ...
    with open(ADULT_SIMPLE_PY, encoding="utf-8") as file:
        ...
        assert (
            HistogramForColumns(["race"])
            in list(inspector_result.dag_node_to_inspection_results.values())[
                0
            ]
        )
        check_to_check_results = inspector_result.check_to_check_results
...
def test_inspector_additional_module() -> None:
    """
    Tests whether the str version of the inspector works
    """
    inspector_result = (
        PipelineInspector.on_pipeline_from_py_file(HEALTHCARE_PY)
        .add_required_inspection(MaterializeFirstOutputRows(5))
...
def test_inspector_additional_modules() -> None:
    """
    Tests whether the str version of the inspector works
    """
    inspector_result = (
...
def assert_healthcare_pipeline_output_complete(
