...
def test_statsmodels_add_constant() -> None:
    """
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_random]
    )
    ...
    expected_lineage_df = DataFrame(
        [
            [numpy.array([0.5, 1.0]), {LineageId(0, 0)}],
            [numpy.array([0.5, 1.0]), {LineageId(0, 1)}],
        ],
        columns=["array", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
...
def test_ols_fit() -> None:
    ...
    expected_train_data = DagNode(
        ...
        OperatorContext(
            OperatorType.TRAIN_DATA, FunctionInfo("statsmodel.api.OLS", "fit")
        ),
    ...
    expected_ols = DagNode(
        ...
        BasicCodeLocation("<string-source>", 10),
        OperatorContext(
            OperatorType.ESTIMATOR, FunctionInfo("statsmodel.api.OLS", "fit")
        ),
    ...
    expected_lineage_df = DataFrame(
        [
            [2.154842811243982, {LineageId(5, 0)}],
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_ols]
    )
    lineage_output = inspection_results_data_source[RowLineage(3)]
    expected_lineage_df = DataFrame(
        [
