...
def test_frame_dropna() -> None:
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_select]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[0.0, {LineageId(0, 0)}], [2.0, {LineageId(0, 1)}]],
        columns=["A", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
        lineage_output.reset_index(drop=True),
...
def test_frame__getitem__series() -> None:
    """
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_project]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[0.0, {LineageId(0, 0)}], [2.0, {LineageId(0, 1)}]],
...
def test_frame__getitem__frame() -> None:
    """
...
def test_frame__getitem__selection() -> None:
    """
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_selection]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[4, 4.0, {LineageId(0, 2)}], [8, 11.0, {LineageId(0, 3)}]],
...
def test_frame__setitem__() -> None:
    """
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[
            expected_project_modify
        ]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [
            ["one", "A", 2, "x", {LineageId(0, 0)}],
...
def test_frame_replace() -> None:
    ...
    expected_modify = DagNode(
        ...
        OptionalCodeInfo(
            CodeReference(4, 13, 4, 40), "df.replace('Medium', 'Low')"
        ),
    )
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_modify]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
...
def test_frame_merge_on() -> None:
    ...
    expected_join = DagNode(
        ...
        BasicCodeLocation("<string-source>", 5),
        OperatorContext(
            OperatorType.JOIN, FunctionInfo("pandas.core.frame", "merge")
        ),
        DagNodeDetails("on 'B'", ["A", "B", "C"]),
        OptionalCodeInfo(
            CodeReference(5, 12, 5, 36), "df_a.merge(df_b, on='B')"
        ),
    )
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_join]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [
            [0, 1, 1.0, {LineageId(0, 0), LineageId(1, 0)}],
            [2, 2, 5.0, {LineageId(0, 1), LineageId(1, 1)}],
        ],
...
def test_frame_merge_left_right_on() -> None:
    """
    Tests whether the monkey patching of ('pandas.core.frame', 'merge') works
    """
    test_code = cleandoc(
    ...
    expected_lineage_df = DataFrame(
        [
            [0, 1, 1, 1.0, {LineageId(0, 0), LineageId(1, 0)}],
            [2, 2, 2, 5.0, {LineageId(0, 1), LineageId(1, 1)}],
        ],
...
def test_frame_merge_index() -> None:
    """
    Tests whether the monkey patching of ('pandas.core.frame', 'merge') works
    """
    test_code = cleandoc(
    ...
    expected_join = DagNode(
        ...
        DagNodeDetails(
            "on left_index == right_index (outer)", ["A", "B", "C", "D"]
        ),
        OptionalCodeInfo(
            CodeReference(5, 12, 5, 82),
    ...
    expected_lineage_df = DataFrame(
        [
            [0, 1, 1.0, 1.0, {LineageId(0, 0), LineageId(1, 0)}],
            [2, 2, 2.0, 5.0, {LineageId(0, 1), LineageId(1, 1)}],
        ],
        columns=["A", "B", "C", "D", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
...
def test_frame_merge_sorted() -> None:
    """
    ...
    expected_join = DagNode(
        ...
        DagNodeDetails("on 'B'", ["A", "B", "C"]),
        OptionalCodeInfo(
            CodeReference(5, 12, 5, 47), "df_a.merge(df_b, on='B', sort=True)"
        ),
    )
    expected_dag.add_edge(expected_a, expected_join)
    expected_dag.add_edge(expected_b, expected_join)
    ...
    expected_lineage_df = DataFrame(
        [
            [5, 1, 1.0, {LineageId(0, 4), LineageId(1, 0)}],
            [8, 2, 11.0, {LineageId(0, 3), LineageId(1, 3)}],
            [4, 4, 5.0, {LineageId(0, 2), LineageId(1, 1)}],
...
def test_groupby_agg() -> None:
    ...
    test_code = cleandoc(
        ...
        df_expected = pd.DataFrame({'group': ['A', 'B', 'C'], 'mean_value': [1.0, 3.0, 3.0]})
    ...
    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_groupby_agg]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [["A", 1.0, {LineageId(1, 0)}], ["B", 3.0, {LineageId(1, 1)}]],
...
def test_series__init__() -> None:
    ...
    expected_node = DagNode(
        ...
        DagNodeDetails(None, ["A"]),
        OptionalCodeInfo(
            CodeReference(3, 12, 3, 48), "pd.Series([0, 2, 4, None], name='A')"
        ),
    )
    compare(extracted_node, expected_node)

    inspection_results_data_source = (
        inspector_result.dag_node_to_inspection_results[expected_node]
    )
    lineage_output = inspection_results_data_source[RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[0.0, {LineageId(0, 0)}], [2.0, {LineageId(0, 1)}]],
