...
from example_pipelines import ADULT_SIMPLE_IPYNB, ADULT_SIMPLE_PY
...
def test_pipeline_executor_py_file(mocker: Any) -> None:
    """
...
def test_pipeline_executor_py_file_without_code_reference_tracking(
    mocker: Any,
) -> None:
    """
    Tests whether the PipelineExecutor works for .py files
    ...
    extracted_dag = _pipeline_executor.singleton.run(
        python_path=ADULT_SIMPLE_PY, track_code_references=False
    ).dag
    expected_dag = get_expected_dag_adult_easy(
        ADULT_SIMPLE_PY, with_code_references=False
    )
    compare(
        networkx.to_dict_of_dicts(extracted_dag),
        networkx.to_dict_of_dicts(expected_dag),
...
def test_pipeline_executor_nb_file(mocker: Any) -> None:
    """
    ...
    extracted_dag = _pipeline_executor.singleton.run(
        notebook_path=ADULT_SIMPLE_IPYNB
    ).dag
...
def test_func_defs_and_loops() -> None:
    """
    ...
    extracted_dag = _pipeline_executor.singleton.run(
        python_code=test_code, track_code_references=True
    ).dag
    ...
    expected_data_source = DagNode(
        ...
        OptionalCodeInfo(
            CodeReference(4, 9, 4, 44), "pd.DataFrame([0, 1], columns=['A'])"
        ),
    )
...
def test_func_defs_and_loops_without_code_reference_tracking() -> None:
    """
    Tests whether the monkey patching of pandas function works
    """
    test_code = get_test_code_with_function_def_and_for_loop()
    ...
    extracted_dag = _pipeline_executor.singleton.run(
        python_code=test_code, track_code_references=False
    ).dag
...
def test_annotation_storage() -> None:
    """
    Tests whether the monkey patching of pandas function works
    """
    test_code = cleandoc(
    ...
    _pipeline_executor.singleton.run(
        python_code=test_code, track_code_references=True
    )


def test_black_box_operation() -> None:
    """
    Tests whether the monkey patching of pandas function works
    """
    test_code = cleandoc(
...
def test_instrument_pipeline_with_code_reference_tracking() -> None:
    """
...
def test_instrument_pipeline_without_code_reference_tracking() -> None:
    """
