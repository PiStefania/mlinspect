...
from demo.feature_overview.no_missing_embeddings import (
    NoMissingEmbeddings,
    NoMissingEmbeddingsResult,
)
...
def test_no_missing_embeddings() -> None:
    """
    ...
    expected_failed_dag_node_with_result = {
        DagNode(
            ...
            OptionalCodeInfo(
                CodeReference(5, 14, 5, 69),
                "MyW2VTransformer(min_count=2, vector_size=2, workers=1)",
            ),
        ): MissingEmbeddingsInfo(2, ["cat_b", "cat_c"])
    }
    expected_result = NoMissingEmbeddingsResult(
        NoMissingEmbeddings(10),
        CheckStatus.FAILURE,
        "Missing embeddings were found!",
        expected_failed_dag_node_with_result,
    )
