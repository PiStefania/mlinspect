...
def test_no_bias_introduced_for_merge() -> None:
    """
...
def get_expected_check_result_merge() -> NoBiasIntroducedForResult:
    ...
    expected_distribution_change = BiasDistributionChange(
        failing_dag_node, False, (0.25 - 0.4) / 0.4, change_df
    )
    expected_dag_node_to_change = {
        failing_dag_node: {"A": expected_distribution_change}
    }
    failure_message = (
        "A Join causes a min_relative_ratio_change of 'A' by -0.37500000000000006, a value below the "
        "configured minimum threshold -0.3!"
    )
    expected_result = NoBiasIntroducedForResult(
        NoBiasIntroducedFor(["A"]),
        CheckStatus.FAILURE,
        failure_message,
        expected_dag_node_to_change,
...
def get_expected_check_result_simple_imputer() -> NoBiasIntroducedForResult:
    ...
    change_df = DataFrame(
        {
            ...
            "ratio_after": [0.75, 0.25, 0.0],
            "relative_ratio_change": [0.5, 0.0, -1.0],
        }
    )
    expected_distribution_change = BiasDistributionChange(
        imputer_dag_node, True, 0.0, change_df
    )
    expected_dag_node_to_change = {
        imputer_dag_node: {"A": expected_distribution_change}
    }
    expected_result = NoBiasIntroducedForResult(
        NoBiasIntroducedFor(["A"]),
        CheckStatus.SUCCESS,
