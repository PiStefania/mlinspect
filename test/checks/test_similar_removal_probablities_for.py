...
from mlinspect.checks._similar_removal_probabilities_for import (
...
def test_removal_probab_for_merge() -> None:
    """
...
def test_removal_probab_simple_imputer() -> None:
    """
...
def test_removal_probab_dropna() -> None:
    """
    ...
    compare(check_result, expected_result)
    assert isinstance(check_result, SimilarRemovalProbabilitiesForResult)
...
def get_expected_check_result_merge() -> SimilarRemovalProbabilitiesForResult:
    ...
    change_df = DataFrame(
        {
            ...
            "removal_probability": [0.0, 0.5, 0.0],
            "normalized_removal_probability": [0.0, 1.0, 0.0],
        }
    )
    expected_probabilities = RemovalProbabilities(
        dag_node, True, 1.0, change_df
    )
    ...
    change_df = DataFrame(
        {
            ...
            "removal_probability": [0.0, 0.0, 1.0],
            "normalized_removal_probability": [0.0, 0.0, 1.0],
        }
    )
    expected_probabilities = RemovalProbabilities(
        dag_node, True, 0.0, change_df
    )
    expected_dag_node_to_change = {dag_node: {"A": expected_probabilities}}
    failure_message = None
    expected_result = SimilarRemovalProbabilitiesForResult(
...
def get_expected_check_result_dropna() -> SimilarRemovalProbabilitiesForResult:
    ...
    change_df = DataFrame(
        {
            ...
            "count_after": [0, 2],
            "removed_records": [2, 1],
            "removal_probability": [1.0, 1.0 / 3.0],
            "normalized_removal_probability": [3.0, 1.0],
        }
    )
    expected_probabilities = RemovalProbabilities(
        dag_node, False, 3.0, change_df
    )
    expected_dag_node_to_change = {dag_node: {"A": expected_probabilities}}
    failure_message = (
        "A Selection causes a max_probability_difference of 'A' by 3.0, a value above the configured "
