...
def test_row_lineage_merge() -> None:
    """
    ...
    expected_lineage_df = DataFrame(
        [[0, 1, {LineageId(0, 0)}], [2, 2, {LineageId(0, 1)}]],
    ...
    expected_lineage_df = DataFrame(
        [[1, 1.0, {LineageId(1, 0)}], [2, 5.0, {LineageId(1, 1)}]],
...
def test_row_lineage_concat() -> None:
    """
    ...
    lineage_output = inspection_results[0][RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[1, "cat_a", {LineageId(0, 0)}], [2, "cat_b", {LineageId(0, 1)}]],
        columns=["A", "B", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
        lineage_output.reset_index(drop=True),
    ...
    lineage_output = inspection_results[1][RowLineage(2)]
    expected_lineage_df = DataFrame(
        [[1, {LineageId(0, 0)}], [2, {LineageId(0, 1)}]],
    ...
    expected_lineage_df = DataFrame(
        [["cat_a", {LineageId(0, 0)}], ["cat_b", {LineageId(0, 1)}]],
    ...
    expected_lineage_df = DataFrame(
        [
            [np.array([1.0, 0.0, 0.0]), {LineageId(0, 0)}],
            [np.array([0.0, 1.0, 0.0]), {LineageId(0, 1)}],
        ],
        columns=["array", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
    ...
    expected_lineage_df = DataFrame(
        [
            [np.array([-1.0, 1.0, 0.0, 0.0]), {LineageId(0, 0)}],
            [
                np.array([-0.7142857142857143, 0.0, 1.0, 0.0]),
                {LineageId(0, 1)},
            ],
        ],
        columns=["array", "mlinspect_lineage"],
...
def test_all_rows_for_op_type() -> None:
    """
    ...
    expected_lineage_df = DataFrame(
        [[1, 1, {LineageId(1, 0)}], [2, 5, {LineageId(1, 1)}]],
        columns=["B", "C", "mlinspect_lineage"],
    )
    pandas.testing.assert_frame_equal(
        lineage_output.reset_index(drop=True),
