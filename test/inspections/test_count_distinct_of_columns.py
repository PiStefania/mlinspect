...
def test_count_distinct_merge() -> None:
    """
    ...
    count_distinct_output = inspection_results[0][
        CountDistinctOfColumns(["A", "B"])
    ]
    expected_count_distinct = {"A": 3, "B": 5}
    compare(count_distinct_output, expected_count_distinct)

    count_distinct_output = inspection_results[1][
        CountDistinctOfColumns(["A", "B"])
    ]
    expected_count_distinct = {"B": 5}
    compare(count_distinct_output, expected_count_distinct)

    count_distinct_output = inspection_results[2][
        CountDistinctOfColumns(["A", "B"])
    ]
