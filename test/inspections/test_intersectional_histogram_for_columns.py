...
def test_histogram_merge() -> None:
    ...
    histogram_output = inspection_results[0][
        IntersectionalHistogramForColumns(["A", "C"])
    ]
    ...
    compare(histogram_output, expected_histogram_1)

    histogram_output = inspection_results[1][
        IntersectionalHistogramForColumns(["A", "C"])
    ]
    expected_histogram_2 = {(None, None): 5}
    compare(histogram_output, expected_histogram_2)

    histogram_output = inspection_results[2][
        IntersectionalHistogramForColumns(["A", "C"])
    ]
    expected_histogram_3 = {
        ("cat_a", True): 2,
        ("cat_b", False): 1,
        ("cat_b", True): 1,
    }
    compare(histogram_output, expected_histogram_3)


def test_histogram_projection() -> None:
    """
