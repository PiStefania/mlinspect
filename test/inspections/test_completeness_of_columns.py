...
def test_completeness_merge() -> None:
    """
    ...
    completeness_output = inspection_results[0][
        CompletenessOfColumns(["A", "B"])
    ]
    expected_completeness = {"A": 0.6, "B": 1.0}
    compare(completeness_output, expected_completeness)

    completeness_output = inspection_results[1][
        CompletenessOfColumns(["A", "B"])
    ]
    expected_completeness = {"B": 0.8}
    compare(completeness_output, expected_completeness)

    completeness_output = inspection_results[2][
        CompletenessOfColumns(["A", "B"])
    ]
...
def test_completeness_projection() -> None:
    """
