...
def test_propagation_merge() -> None:
    """
    ...
    expected_df = DataFrame(
        [["cat_a", 1, "cat_a"], ["cat_b", 2, "cat_b"]],
        columns=["A", "B", "mlinspect_A"],
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
        expected_df.reset_index(drop=True),
    )

    propagation_output = inspection_results[1][ColumnPropagation(["A"], 2)]
    expected_df = DataFrame(
        [[1, 1.0, None], [2, 5.0, None]], columns=["B", "C", "mlinspect_A"]
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
        expected_df.reset_index(drop=True),
    ...
    expected_df = DataFrame(
        [["cat_a", 1, 1.0, "cat_a"], ["cat_b", 2, 5.0, "cat_b"]],
...
def test_propagation_projection() -> None:
    """
    ...
    propagation_output = inspection_results[0][ColumnPropagation(["A"], 2)]
    expected_df = DataFrame(
        [["cat_a", 1, 2, "cat_a"], ["cat_b", 2, 2, "cat_b"]],
        columns=["A", "B", "C", "mlinspect_A"],
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
    ...
    propagation_output = inspection_results[1][ColumnPropagation(["A"], 2)]
    expected_df = DataFrame(
        [[1, 2, "cat_a"], [2, 2, "cat_b"]], columns=["B", "C", "mlinspect_A"]
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
        expected_df.reset_index(drop=True),
    ...
    propagation_output = inspection_results[2][ColumnPropagation(["A"], 2)]
    expected_df = DataFrame(
        [[2, "cat_a"], [2, "cat_b"]], columns=["C", "mlinspect_A"]
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
        expected_df.reset_index(drop=True),
...
def test_propagation_score() -> None:
    """
    Tests whether ColumnPropagation works for projections
    """
    test_code = cleandoc(
    ...
    propagation_output = inspection_results[14][
        ColumnPropagation(["cat_col"], 2)
    ]
    expected_df = DataFrame(
        [[0, "cat_a"], [1, "cat_b"]], columns=["array", "mlinspect_cat_col"]
    )
    pandas.testing.assert_frame_equal(
        propagation_output.reset_index(drop=True),
        expected_df.reset_index(drop=True),
