...
class AlibiCallInfo:
    ...
    parent_nodes_ig = None
    parent_nodes_ale = None
...
class AlibiPatching:
    ...
    def patched__init__(self, model, layer = None, target_fn = None, method = "gausslegendre", n_steps = 50,
        ...
        def execute_inspections(_, caller_filename, lineno, optional_code_reference, optional_source_code):
            ...
            add_dag_node(dag_node, call_info_singleton_alibi.parent_nodes_ig, backend_result)
    ...
    @gorilla.patch(alibi.explainers.ALE, name="__init__", settings=gorilla.Settings(allow_hit=True))
    def patched__init___ale(self, predictor, feature_names=None, target_names=None, check_feature_resolution=True,
                 low_resolution_threshold=10, extrapolate_constant=True, extrapolate_constant_perc=10.,
                 extrapolate_constant_min=0.1, mlinspect_caller_filename=None,
        ...
        # pylint: disable=no-method-argument, attribute-defined-outside-init
        original = gorilla.get_original_attribute(alibi.explainers.ALE, '__init__')
        ...
        self.mlinspect_optional_source_code = mlinspect_optional_source_code
        self.mlinspect_fit_transform_active = mlinspect_fit_transform_active
        self.mlinspect_non_data_func_args = {'predictor': predictor, 'feature_names': feature_names, 'target_names': target_names,
                                             'check_feature_resolution': check_feature_resolution, "low_resolution_threshold": low_resolution_threshold,
                                             "extrapolate_constant": extrapolate_constant, "extrapolate_constant_perc": extrapolate_constant_perc,
        ...
        def execute_inspections(_, caller_filename, lineno, optional_code_reference, optional_source_code):
            ...
            dag_node = DagNode(call_info_singleton_alibi.mlinspect_explainer_node_id,
                               ...
                               DagNodeDetails("Neural Network", []),
                               get_optional_code_info_or_none(optional_code_reference, optional_source_code))
            add_dag_node(dag_node, call_info_singleton_alibi.parent_nodes_ale, backend_result)

        return execute_patched_func_no_op_id(original, execute_inspections, self, **self.mlinspect_non_data_func_args)

    @gorilla.patch(alibi.explainers.ALE, name='explain', settings=gorilla.Settings(allow_hit=True))
    def patched_explain_ale(self,  X, features=None, min_bin_points=4, grid_points=None):
        """ Patch for ('alibi.explainers.ALE', 'explain') """
        # pylint: disable=no-method-argument
        original = gorilla.get_original_attribute(alibi.explainers.ALE, 'explain')
        args = {'X': X, 'features': features, 'min_bin_points': min_bin_points, 'grid_points': grid_points,}
