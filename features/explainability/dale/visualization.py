...
def feature_effect_plot(
    ...
    bins_under_limit = np.argwhere(
        params["points_per_bin"] < min_points_per_bin
    )
    ...
    y, estimator_var, var = eval_f(x, feature)
...
def feature_effect(
    ...
    if first_empty is not None:
        added_line = 0.3 * (np.max(y) - np.min(y))
        ax1.vlines(
            x=[limits[first_empty], limits[first_empty + 1]],
            ymin=np.min(y) - added_line,
            ymax=np.max(y) + added_line,
            colors="red",
            alpha=0.7,
...
def fe_all(dale, ale, pdp, mplot, feature, ale_gt=None):
    dale_first_empty_bin = dale.parameters["feature_" + str(feature)][
        "first_empty_bin"
    ]
    dale_limits = dale.parameters["feature_" + str(feature)]["limits"]

    ale_first_empty_bin = ale.parameters["feature_" + str(feature)][
        "first_empty_bin"
    ]
    ...
    # ALE gt
    x = np.linspace(left_lim - 0.01, right_lim + 0.01, 10000)
    ...
    # MPlot
    y = mplot.eval(x, feature, tau=0.1)
    plt.plot(x, y, "y--", label="MPlot")

    if first_empty_bin is not None:
        plt.axvspan(
            xmin=dale_limits[first_empty_bin],
            xmax=x[-1],
            ymin=np.min(y),
            ymax=np.max(y),
            alpha=0.2,
